...
class Image:
    def __init__(self,path,arguments):
        ...
        self.max_dist = arguments.max_dist
    ...
    def scale_pixels(self):
        ...
        if self.y_type == "log":
            ...
            #generating converting scale
            y_conv_scale = []
            tmp_value = min(self.y_labels)
            print tmp_value
            steps = math.log(max(self.y_labels)/min(self.y_labels),10)+1
            lin_scale = np.linspace(tmp_value,max(self.y_labels),steps)
            print tmp_value, steps, lin_scale

            i = 0
            while tmp_value != max(self.y_labels):
                y_conv_scale.append([tmp_value,lin_scale[i]])
                tmp_value *= 10
                i += 1
        ...
        for i in xrange(len(self.lines)): #iterate over lines
            for j in xrange(len(self.lines[i])): #iterate over line - pixels
                ...
                if self.y_type == "log":
                    ...
                    #todo: improve
                    smaller = [item for item in y_conv_scale if item[1] <= tmp_pix.y]
                    larger = [item for item in y_conv_scale if item[1] >= tmp_pix.y]
                    down = min(smaller,key=lambda item: abs(tmp_pix.y-item[1]))
                    up = min(larger,key=lambda item: abs(tmp_pix.y-item[1]))
                    print down,up
                    #raw_input("down, up")

                    distance = (tmp_pix.y-down[1])/float(up[1]-down[1])
