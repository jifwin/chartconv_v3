    ...
    def scale_pixels(self):
        ...
        for i in xrange(len(self.lines)): #iterate over lines
            ...
            for j in xrange(len(self.lines[i])): #iterate over line - pixels
                tmp_pix = self.lines[i][j]
                #print str(tmp_pix)
                #print self.labels[0], self.labels[2]
                #old scaling todo:#tmp_pix.x = self.labels[0]+x_range*tmp_pix.x/float(max_x_point)
                #old scaling #tmp_pix.y = self.labels[2]+y_range*tmp_pix.y/float(max_y_point)
                print self.labels[2],scale_y,tmp_pix.y
